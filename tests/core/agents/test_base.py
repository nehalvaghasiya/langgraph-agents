...
from core.agents.base import AgentState, BaseAgent
...
@pytest.mark.parametrize(
    "tools,system",
    [([], None), ([DummyTool("t1")], "sys"), ([DummyTool("t1"), DummyTool("t2")], None)],
)
...
@pytest.mark.parametrize(
    "tool_calls,result", [([], False), ([{"name": "t1", "id": "1", "args": {}}], True)]
)
...
@pytest.mark.parametrize(
    "tool_calls,tool_names,expected",
    [
...
def test_take_action_tool_exception():
    ...
    assert "messages" in result
    assert any(
        "Exception" in m.content or "error" in m.content.lower() or isinstance(m.content, str)
